<template>
  <main class="cart">
    ...
    <section v-if="cart.length">
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td colspan="3"></td>
            <td class="total">总计: ¥{{ totalPrice.toFixed(2) }}</td>
          </tr>
        </tbody>
      </table>
    </section>
    <section v-else class="emptycart">
      ...
      <h3>还没有添加商品</h3>
      <nuxt-link exact to="/">
        <button class="d-btn d-btn-primary">前往添加</button>
      </nuxt-link>
    </section>
    <nuxt-link to="/"><button class="d-btn d-btn-primary">返回主页面</button></nuxt-link>
  </main>
</template>
